...
%     freqList = logFreqList(fpass,30);
    ...
    % DKL addition to match frequencies I've been using on my old data from Josh's lab
    ...
    spikeLFPcorr_subject_dir = fullfile(analysis_storage_dir, [analysisConf.subjects__name '_spike_LFP_corr']);
    spikeLFPcorr_session_dir = fullfile(spikeLFPcorr_subject_dir, analysisConf.sessionNames{iNeuron});
...
%     sigma = round(mean(diff(ts))/3,3); % use mean ISI?
    [s,binned,kernel] = spikeDensityEstimate_Fs(ts,endTs,sigma,Fs);
