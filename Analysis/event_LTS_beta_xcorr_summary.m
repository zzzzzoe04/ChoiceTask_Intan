...
for i_ratDir = 1 : length(ratDirs)
    ...
    for i_sessionDir = 1 : length(sessionDirs)
        ...
        iEvent = 4;
        curEventBeta = mean(squeeze(corrData{iUnit}.periEventBeta(iEvent,:,:)));
        cur_periEventBeta_z = (cur_periEventBeta - mean(corrData{iUnit}.periRandomBeta,1))./std(corrData{iUnit}.periRandomBeta,0,1);
        plot(t,cur_periEventBeta_z);
        hold on
        for iUnit = 1 : numValidFiles
            net_LTShist = net_LTShist + corrData{iUnit}.LTShist;
            toPlot = mean(squeeze(corrData{iUnit}.LTShist(iEvent,:,:)));
            
            plot(hist_t,toPlot);
        end
            figure(h_overlay_fig);
