...
for iRat = 1 : length(ratIDs)
    ...
    %             tr_trialsOnly = extractTrial_ts(ts, trials);
        ...
        if isNewSession
            ...
            hilbertMean = mean(abs(sevHilbert));
            hilbertSTD = std(sevHilbert);
            betaMean = mean(betaPower);
            betaStd = std(betaPower);
        end

        % filenames to store the analyzed LFP data
        periSpike_beta_name = [neuronName '_periSpikeBeta_all.mat'];
        tsScalo_subject_dir = fullfile(analysis_storage_dir, [analysisConf.subjects__name '_spike_triggered_scalos']);
        ...
        periSpikeBeta = calcPeriSpikeLFP( tsLTS(burstIdx), abs(sevHilbert), tWindow, Fs );
        
        rand_ts = tWindow + (rand(numRandomTs,1) * (length(sevFilt)/Fs - 2*tWindow));
        periRandomBeta = calcPeriSpikeLFP( rand_ts, abs(sevHilbert), tWindow, Fs );
