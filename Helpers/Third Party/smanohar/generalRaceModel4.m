...
%                  This can be a scalar, or a row specifying the start time
%                  of each signal, or a matrix specifying the start time of
%                  each signal on each trial. In this case the number of
%                  rows must equal nTrials.
%                  Default = zeros(1,N).
...
%                 DELAY    = 6   triggers cause a delay or pause
%
...
if ~isfield(p,'startTime') % expand for each signal
    p.startTime = zeros(1,NS);
elseif length(p.startTime)==1;
    p.startTime = p.startTime * ones(1,NS);
end
if size(p.startTime,1)==1 % expand for each trial in simulation
  p.startTime = repmat(p.startTime,N,1);
end
...
SET_ACCU=4;  % triggers set the accumulator value
ADD_ACCU=5;  % triggers add on to the accumulator value
PAUSE   =6;  % triggers pause the accumulator
...
% spmd % parallelise!
    ...
    processCompleted = false(N,NS);   % boolean for each process 
    ...
    while ~allEnded
        ...
        deltaA = max(0,repmat(t,1,NS)-timegapbefore) .* R;
        ...
        iTrig(t==inf)=nan;
        timegapbefore=max(0,timegapbefore-repmat(t,1,NS));  % update time gaps if needed
        ...
        switch p.mode 
            ...
            case PAUSE
                timegapbefore(~nochange) = newR(~nochange);
                R(~nochange & newR==inf) = 0; % time gap of zero --> freeze process
        end
