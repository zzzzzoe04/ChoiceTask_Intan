...
% H = PLOTN( X, Y, .... )
%
% Same as plot, but for 3-, 4- or 5-dimensional arrays.
...
if isequal(plotfun,@errorBarPlot)
  THREED = true;  % should we pass 3D data to the plot fun?
else
...
if length(sz)<4, sz=[sz ones(1,4-length(sz))]; end % make sure 4 dimensions

%%%% do plot
if isempty(linestyle) % no linestyles; dim 2=lines, dim3/4 = subplots
  for i=1:sz(3)  % rows of subplots
    for j=1:sz(4)   % columns of subplots
      subplot(sz(3),sz(4), (i-1)*sz(4)+j);
  ...
  if length(sz)==4, sz(5)=1; end % need 5 dimensions
  if sz(3) > length(linestyle), warning('Please provide %g line styles for dimension 3',sz(3)); end
  for i=1:sz(4)  % rows of subplots
    for j=1:sz(5)   % columns of subplots
      subplot(sz(4),sz(5), (i-1)*sz(5)+j);
      for k=1:sz(3) % for each line style
