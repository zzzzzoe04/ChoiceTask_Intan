...
[fittingMode fiddle POPULATION GENERATIONS N_MODEL_TRIALS, N_MODEL_AVERAGES] = parsepvpairs( ...
    {'fittingMode', 'fiddle', 'POPULATION', 'GENERATIONS', 'N_MODEL_TRIALS', 'N_MODEL_AVERAGES' }, ...
    {'Genetic',      0 ,       500,          30,            2000,             5 }, varargin{:});
...
solution=struct();
  ...
  % run minimum search
  %
  if(FMINSEARCH) % FMINSEARCH (original version)
  ...
  elseif(GENETIC)  % GENETIC ALGORITHM
    [p1,LL,flags,result,population, scores] = ga( ...
                 @(p) -loglikelihood(p, RTs, conditionVector, ...
                                     baseRaceParameters, varyParametersFunction, ...
                                     N_MODEL_TRIALS, N_MODEL_AVERAGES),...
                 length(p0),...
                 gaoptimset('PopulationSize',POPULATION, ...
                     'EliteCount', floor(POPULATION/10), ... % 10% of population are elite
  ...
  elseif (ANNEAL)     % SIMULATED ANNEALING
    ...
    [p1, LL] = anneal_sanjay ...
      ...
      struct(   'InitTemp',1 , ... annealing parameters
                'StopTemp',0.01 , ... (1e-8)
                'Generator', Generator, ...
                'Verbosity',2, ...
                'MaxTries', 300, ...  maximum tries per temperature (300)
                'MaxSuccess', 20, ... maximum successful tries within one temp (20)
                'CoolSched', @(T)( 0.8*T ) ... cooling rate (0.8)
      ));
  else % NO FITTING - TEST OUT INITIAL PARAMETERS
...
function LL = loglikelihood1(params, RTs, conditions, modelParams, paramFn, N_MODEL_TRIALS)
  ...
  for(c=1:length(ucond))
    ...
    if size(conditions,2)>1
      p = paramFn(modelParams(c), params, conditions(:,2:end));
    else
