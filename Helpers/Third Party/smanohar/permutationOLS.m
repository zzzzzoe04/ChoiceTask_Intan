function [ t_test_result, p_vals, t_statistics, t_threshold, hplot ] ...
         = mixedEffectPermutation(Y,X,C, group, varargin )
% function [  t_test_result, p_vals, t_statistics, t_threshold, hplot ]  ...
%                  = mixedEffectPermutation( Y, X, C,  GROUP,  ['param',value]...)
%
...
for p=1:NPERMS % for each permutation 
  ...
  for j=1:length(ug)           % then for each group:
    ...
    if ~onesample_t            % standard t test   -- H0: "gradient == 0"
      ...
      pX( rows,: ) = ((rand(size(X))<0.5)*2-1) .* pX( rows,: ); % if so, then choose -1 or +1 randomly
    end
  end
...
if TWO_TAILED
  ...
  p_vals        = mean( bsxfun(@lt, t_wg_max , permute(t_statistics, [1,3,2])) , 2  ); 
end
