...
SMOOTH_IN_CLEANING = false; % apply a minor smoothing while cleaning, so that points around bad points are excluded.
...
for(i=1:length(s))  % for each trial
  if ~isempty(s(i).pos) % are there any samples?
    ...
    if ~isempty(v) && ~saccadeonly % 2014 added no cleaning if saccadeonly is requested
      % clean up bad data
      % was 1.5
      BLINKSPD=0.5; % speed above which data is removed around nans. Normally 1.5
      nanC=nan+nan*1j;
      ...
      if SMOOTH_IN_CLEANING      % this makes samples surrounding NaN
        smooth(spd,3,'boxcar');  % also into NaN - expanding the "bad" region a little.
    ...
    if ~pupil && ~isempty(s(i).saccade) &&  ~isempty(scStart) % we have nonempty saccades?
      for j=1:length(scStart)                                 % go through each saccade
        ...
        if mean(isnan(p(scEnd(j):nextgood)))>0.95 || scEnd(j)==length(p), % if 95% of future samples are NaN
  ...
  if(browse && length(scStart)>0)
    ...
    hold off;
    keyboard % use this to stop and allow inspection
    %pause; % use this to allow keypress-to-advance
  end
  ...
  if RETURN_TRACES % Do we want to return the individual traces for trials?
    ...
    if(stretch==0)               % no stretch: add nans at the end
      ...
      if(length(p)==1)           % so that each trial appears tp
        r(i,:)=p;                % have the same number of samples.
    ...
    %%%% Smoothing
    if smoothing & length(r)>smoothing          % apply smoothing to traces
