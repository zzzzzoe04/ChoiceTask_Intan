...
%     grid on;
    ...
    subplot(rows,cols,prc(cols,[1,iEvent]));
    imagesc(t_xcorr,1:numel(freqList),squeeze(mean(all_xcorrBands_events(:,iEvent,:,:),1))');
    hold on;
    colormap(gca,jet);
    set(gca,'YDir','normal');
    xlim([-1 1]);
    ...
    if iEvent == 4
        shuff_caxisVals = caxis;
    end
    grid on;
end
for iEvent = 1:7
    subplot(rows,cols,prc(cols,[1,iEvent]));
    caxis(xcorr_caxisVals);
    if iEvent == 7
        cb = cbAside(gca,'mean xcorr','k');
        cb.Ticks = caxis;
    end
    subplot(rows,cols,prc(cols,[2,iEvent]));
    caxis(shuff_caxisVals);
    if iEvent == 7
        cb = cbAside(gca,'mean shuffle sign.','k');
        cb.Ticks = caxis;
    end
end
set(gcf,'color','w');
savePath = '/Users/mattgaidica/Documents/Data/ChoiceTask/LFPs/perievent/xcorrBySession';
saveas(h,fullfile(savePath,'allSessions_xcorrBySession.png'));
close(h);
%%
