%%
% This wrapper was started on 20230602 by JM and is NOT complete as of
%       20230602

intan_parent_directory = 'X:\Neuro-Leventhal\data\ChoiceTask';
rats_with_intan_sessions = find_rawdata_folders(intan_parent_directory); % kilosort output .npy files are saved in the individual rawdata folder sessions with all ephys data
%% 

% Use make_PETHs_required_variables.m as an example of what variables are
%       needed in the workspace
% need the trial structure for that session
% run this analysis, generating an array that has the timestamps for
% correct trials for all eventFieldnames
...
for i_rat = 1 : length(rats_with_intan_sessions)
    ...
    for i_sessionfolder = 1 : length(intan_folders)
        ...
        session_clust_timestamps_folder = create_cluster_timestamps_data_folder(rd_metadata, intan_parent_directory);
        ratID = rd_metadata.ratID;
        intan_session_name = rd_metadata.session_name;
        session_name = rd_metadata.session_name;
        
%         if any(strcmp(session_path, sessions_to_ignore)) % shouldn't need
%                       % this on this analysis for now
%             continue;
%         end        
       ...
       parentFolder = fullfile(intan_parent_directory, ...
        ...
        spike_clusters_fname = fullfile(intan_folders{i_sessionfolder}, 'spike_clusters.npy'); % need this line if we're reading in kilosort files and reading in .npy information from phy
        spike_times_fname = fullfile(intan_folders{i_sessionfolder}, 'spike_times.npy'); % need this line if we're reading in kilosort files and reading in .npy information from phy
        ...
        if ~exist(spike_clusters_fname, 'file') % need this line if we're reading in kilosort files and reading in .npy information from phy
            sprintf('no spike_clusters input file for %s', session_folder);
            continue
        end

        if ~exist(spike_times_fname, 'file') % need this line if we're reading in kilosort files and reading in .npy information from phy
            sprintf('no spike_times input file for %s', session_folder);
            continue
        end
        ...
        [ trialEventParams ] = getTrialEventParams(trialType); % start with correctgo
        trIdx = extractTrials(trials, trialEventParams);
        num_trials = length(trIdx);
        
        % UP THROUGH THIS POINT WE NEED FOR PETHS DATA - including trials
        % structure, read in spike_clusters and spike_times.

        
        % AS OF 20230602 TRYING TO DETERMINE IF WE NEED THIS OR HOW TO EDIT
        % FOR GENERATING A STRUCTURE ARRAY OF SESSION DATA

        % THIS WAS WRITTEN FOR EVENT TRIGGERED LFPS BUT MAY BE ABLE TO EDIT FOR GENERATING A STRUCTURE
        % FOR event triggered peths - working here
        for i_event = 1 : num_events
                    % edit code here for getting event_triggered_peths
                    % for each separate event
                   
                   % create a filename to save the trials_structure 
                   peths_fname_to_save = char(strcat(session_name, '_', eventFieldnames{i_event}, '_', trialType, '_', 'event_triggered_peths', '.mat')); % add in ''_', sprintf('trial%u.pdf', trial_idx)' should you want to save files individually
                   peths_full_name = fullfile(session_clust_timestamps_folder, peths_fname_to_save);
                   
                   if exist(peths_full_name, 'file')
                        continue
                   end

                   % Catch for troubleshooting the data to NOT run through all of the folders and
                   % create the data if the files are already made. This will not create 5 pages of 5 different random trials.
                   ...
                   % this function adds 2 fields to the trials structure to identify bad sites
                    ...
                    unit_struct = sort_ts_to_units(spike_clusters, spike_times, Fs);
                    save(peths_full_name, 'unit_struct', 'trial_ts', '-mat');
