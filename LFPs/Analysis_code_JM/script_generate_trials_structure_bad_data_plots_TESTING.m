...
valid_trials_original_folder = find_trials_struct_original_folders(intan_parent_directory);
...
%%
        %Testing just loading in the trials structure here
...
for i_ratfolder = 1 : length(rats_with_intan_sessions)
    ...
    intan_folders = rats_with_intan_sessions(i_ratfolder).intan_folders;
...
%         end
            ...
            if contains(session_name, sessions_to_ignore) || contains(session_name, sessions_to_ignore1) || contains(ratID, 'DigiInputTest') % Just always ignore these sessions. R0411 no data, DigitInputTest is t est files
                continue;
            end
...
%         % Start of generation of trials structure
            [session_folder, ~, ~] = fileparts(intan_folders{i_sessionfolder}); % even with loading in trials structure, still need logData but it is quick to load.
...
%         % End of calculating trials structure (original without threshold info)
            ...
            %                             switch trials_validchannels_marked(i_trial).is_channel_valid_ordered(i_row) 
                                        ...
                                        plot_channel_lfps = plot(t, channel_lfps(i_row, :), plot_color); % change to log10 -- plot(f, 10*log10(power_lfps(:,1)))
                                        set(gca, 'ylim',y_lim);
                                        grid on
