...
for i_ratfolder = 1 : length(valid_rat_folders)
    ...
    % extract the ratID and session name from the LFP file
        ...
        if ~exist(power_fn, 'file')
            [ordered_lfp, intan_site_order, NNsite_order] = lfp_by_probe_site(lfp_data, probe_type); % change probe_type to a list of rats with each probe_type? Add a variable?
