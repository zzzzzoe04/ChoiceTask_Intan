...
% incorrect trials with no tone do not have proper timestamps structure,
% therefore the tsPeth is not formatted/contains empties
tidx_contra_incorrect = find([trials.movementDirection] == 1 & [trials.tone] == 2);
tidx_ipsi_incorrect = find([trials.movementDirection] == 2 & [trials.tone] == 1);
...
for iEvent = 1:numel(eventFieldnames)
    ...
    tmp1 = mean(zCounts_co(tidx_contra_correct,:),1);
    all_tidx_contra_correct(iNeuron,iEvent,:,:) = tmp1;
    
    tmp2 = mean(zCounts_co(tidx_ipsi_correct,:),1);
    all_tidx_ipsi_correct(iNeuron,iEvent,:,:) = tmp2;
   
    tmp3 = mean(zCounts_co(tidx_contra_incorrect,:),1);
    all_tidx_contra_incorrect(iNeuron,iEvent,:,:) = tmp3;
   
    tmp4 = mean(zCounts_co(tidx_ipsi_incorrect,:),1);
    all_tidx_ipsi_incorrect(iNeuron,iEvent,:,:) = tmp4;
    ...
    if doplot
        ...
        hold on;
        grid on;
        plot(x,smooth(tmp1,nSmooth),'linewidth',2);
        plot(x,smooth(tmp2,nSmooth),'linewidth',2);
        plot(x,smooth(tmp3,nSmooth),'linewidth',2);
        plot(x,smooth(tmp4,nSmooth),'linewidth',2);
